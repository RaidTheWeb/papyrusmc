...
public class DedicatedServer extends MinecraftServer implements IServer
{
    ...
    public boolean startServer() throws IOException
    {
        ...
        thread.start();
        LOGGER.info("Starting papyrus server version 1.12");

        if (Runtime.getRuntime().maxMemory() / 1024L / 1024L < 512L)
        {
            LOGGER.warn("To start the server with more ram, launch it as \"java -Xmx1024M -Xms1024M -jar papyrus_server.jar\"");
        }
        ...
        {
            LOGGER.info("You are required to accept the Mojang EULA to proceed, go to eula.txt for more info.");
        ...
        else
        {
            ...
            this.setMOTD(this.settings.getStringProperty("motd", "Papyrus Powered Minecraft Server"));
            ...
            LOGGER.info("Starting Papyrus Minecraft server on {}:{}", this.getServerHostname().isEmpty() ? "*" : this.getServerHostname(), Integer.valueOf(this.getServerPort()));
            ...
            {
                LOGGER.warn("FAILED TO BIND PORT");
                LOGGER.warn("The error that occurred: {}", ioexception.toString());
                LOGGER.warn("Another program (plausibly another server) may be running on this port");
                return false;
            }
            ...
            if (!this.isServerInOnlineMode())
            {
                ...
                LOGGER.warn("Papyrus will not authenticate using the Mojang authentication servers.");
                LOGGER.warn("This mode will allow players to join without an internet connection (on a local computer) but players can choose any username they wish. This opens up your server to be accessable via `cracked` clients such as TLauncher.");
                LOGGER.warn("Unless you absolutely must keep offline mode, set \"online-mode\" to \"true\" in the server.properties file.");
            }
            ...
            else
            {
                ...
                LOGGER.info("Server has started! Type \"help\" or \"?\" for information on commands.", s3);
